...
## Versioning:
- for RN >= 0.40.0, use v3+ (e.g. react-native-google-places@beta)
...
```
npm i react-native-google-places@beta --save
react-native link react-native-google-places
```
OR

```
yarn add react-native-google-places@beta
react-native link react-native-google-places
```
...
- Set the `minSdkVersion` for your build at 21 or above. Add the following in your `/android/build.gradle` file:
...
- **placeFields** is an **`Array`** of `String` such as `placeID`, `location`, `name`, `address`, `types`, `openingHours`, `plusCode`, `rating`, `userRatingsTotal`, `viewport`, `website`, `phoneNumber`, `plusCode` and `addressComponents` _(iOS only)_.
